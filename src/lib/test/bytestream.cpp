...
TEST(ByteStream, U64) {
	...
	bs.seek(0);
	ASSERT_EQ(0x1706f5e4d3c2b1a0, bs.readU64(BE));
	uint64_t t;
	bs.read(&t, LE);
	ASSERT_EQ(0x0807060504030201, t);
}
...
TEST(ByteStream, I64) {
	...
	bs.seek(0);
	ASSERT_EQ(3385978729552412671, bs.readI64(BE));
	int64_t t;
	bs.read(&t, LE);
	ASSERT_EQ(-70080650589044223, t);
}
...
TEST(ByteStream, Double) {
	...
	bs.write(1234.5, LE);
	bs.seek(0);
	ASSERT_EQ(0x23f3c89f0c24fec0, bs.readU64(BE));
	double t;
	bs.read(&t, LE);
...
TEST(ByteStream, readVec) {
	narf::ByteStream bs;
	bs.write(0x0102030405060708, LE);
